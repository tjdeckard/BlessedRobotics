    ...
    public class USIModuleHitch : PartModule
    {
        ...
        protected void LockToggle(bool isLocked)
        {
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.LockToggle: Hinge is " + (isLocked ? "" : "not ") + "locked.");
        ...
        {
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.VesselOnRails called.");
        ...
        {
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.VesselOffRails called.");
        ...
        public void VesselCreated(Vessel v)
        {
            if (GameSettings.VERBOSE_DEBUG_LOG && this.vessel == v)
                Debug.Log("[Konstruction] Hitch.VesselCreated called.");
        ...
        public void PartJointBreak(PartJoint joint, float something)
        {
            ...
            if (iAmParent || iAmTarget || iAmHost)
            {
                if (GameSettings.VERBOSE_DEBUG_LOG)
                {
                    Debug.Log("[Konstruction] Hitch.PartJointBreak called on vessel rooted at: " + vessel.rootPart.name);
                    Debug.Log("[Konstruction] Hitch.PartJointBreak: I am " + (iAmHost ? "host " : "") + (iAmParent ? "parent " : "") + (iAmTarget ? "target " : ""));
                }
        ...
        public void PartDocked(GameEvents.FromToAction<Part, Part> eventData)
        {
            ...
            {
                if (GameSettings.VERBOSE_DEBUG_LOG)
                    Debug.Log("[Konstruction] Hitch.PartDocked called. From: " + eventData.from.name + ", To: " + eventData.to.name);
        ...
        public override void OnAwake()
        {
            ...
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.OnAwake called.");
        ...
        {
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.OnDestroy called.");
        ...
        public override void OnStart(StartState state)
        {
            ...
            if (GameSettings.VERBOSE_DEBUG_LOG)
                Debug.Log("[Konstruction] Hitch.OnStart called.");
            ...
            try
            {
                ...
                if (_fixedMesh != null && _movableMesh != null)
                {
                    if (GameSettings.VERBOSE_DEBUG_LOG)
                    {
                        Debug.Log("[Konstruction] Hitch.OnStart: Found fixed mesh!");
                        Debug.Log("[Konstruction] Hitch.OnStart: Found movable mesh!");
                    }
            ...
            {
                Debug.LogError("[Konstruction] Hitch.OnStart encountered an error: " + ex.Message);
            }
        }
        ...
        public override void OnStartFinished(StartState state)
        {
            ...
            try
            {
                ...
                if (GameSettings.VERBOSE_DEBUG_LOG)
                {
                    Debug.Log("[Konstruction] Hitch.OnStartFinished: We are " + (attachmentNode == null ? "not " : string.Empty) + "attached to another part.");
                    if (attachmentNode != null)
                        Debug.Log("[Konstruction] Hitch.OnStartFinished: We are attached to " + attachmentNode.Parent.name + ".");
                }
                ...
                {
                    if (GameSettings.VERBOSE_DEBUG_LOG)
                        Debug.Log("[Konstruction] Hitch.OnStartFinished: Found root Rigidbody!");
            ...
            {
                Debug.LogError("[Konstruction] Hitch.OnStartFinished encountered an error: " + ex.Message);
            }
        }
        ...
        public override void OnUpdate()
        {
            ...
            if (HighLogic.LoadedSceneIsFlight && part.State != PartStates.DEAD && !_isVesselOnRails )
            {
                ...
                {
                    if (GameSettings.VERBOSE_DEBUG_LOG)
                        Debug.Log("[Konstruction] Hitch.OnUpdate: Looking for child attach nodes.");
        ...
        protected bool ReattachChildren()
        {
            ...
            if (HighLogic.LoadedSceneIsFlight && part.State != PartStates.DEAD)
            {
                ...
                {
                    if (GameSettings.VERBOSE_DEBUG_LOG)
                        Debug.Log("[Konstruction] Hitch.ReattachChildren: Looking for child attach nodes.");
        ...
        protected bool ReattachChild(ConfigurableJoint joint)
        {
            ...
            {
                Debug.LogError("[Konstruction] Hitch.ReattachChild encountered an error: " + ex.Message);
